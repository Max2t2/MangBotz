...
let caption = `Tingkat ke *${args[0]}an* \n@${who.split("@")[0]} \nSebesar *${angka.getRandom()}.${angka2.getRandom()}%*`
  await conn.sendButton(m.chat, botdate, caption, await(await fetch(hwaifu.getRandom())).buffer(), [['MENU', '/menu'],['Profile', '.profile']], m, { mentions: conn.parseMention(caption), fileLength: fsizedoc, seconds: fsizedoc, contextInfo: {
          externalAdReply :{
    mediaUrl: sig,
    mediaType: 2,
    description: wm, 
