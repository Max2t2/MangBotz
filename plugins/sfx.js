...
try {
  let gas = await fetch(`http://www.myinstants.com/api/v1/instants/?format=json&page=${args[0]}`)
    ...
    Object.values(dapet).map((v, index) => {
        listSections.push([cmenub + " " + ++index, [
            [v.name, usedPrefix + "get " + v.sound, "\n*Slug:* " + v.slug]
        ]])
    })
    return conn.sendList(m.chat, htki + " 📺 SFX Search 🔎 " + htka, `⚡ Silakan pilih SFX di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`, author, `☂️ SFX Disini ☂️`, listSections, m)
} catch (e) {
...
try {
    let listSections = []
    Object.keys(hasil).map((v, index) => {
        listSections.push([cmenub + " " + ++index, [
            [hasil[v], usedPrefix + "get https://raw.githubusercontent.com/AyGemuy/HAORI-API/main/audio/" + hasil[v] + ".mp3", "\n*By:* " + author]
        ]])
    })
    return conn.sendList(m.chat, htki + " 📺 SFX Search 🔎 " + htka, `⚡ Silakan pilih SFX di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`, author, `☂️ SFX Disini ☂️`, listSections, m)
} catch (e) {
...
let listSections = []
    Object.keys(hasil).map((v, index) => {
        listSections.push([cmenub + " " + ++index, [
            ["Sound ke- " + hasil[v], usedPrefix + "get https://raw.githubusercontent.com/AyGemuy/HAORI-API/main/sound/sound" + hasil[v] + ".mp3", "\n*By:* " + author]
        ]])
    })
    return conn.sendList(m.chat, htki + " 📺 SFX Search 🔎 " + htka, `⚡ Silakan pilih SFX di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`, author, `☂️ SFX Disini ☂️`, listSections, m)
} catch (e) {
...
let listSections = []
    Object.keys(hasil).map((v, index) => {
        listSections.push([cmenub + " " + ++index, [
            ["Sound ke- " + hasil[v], usedPrefix + "get https://raw.githubusercontent.com/WH-MODS-BOT/Sounds/main/sound" + hasil[v] + ".mp3", "\n*By:* " + author]
        ]])
    })
    return conn.sendList(m.chat, htki + " 📺 SFX Search 🔎 " + htka, `⚡ Silakan pilih SFX di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`, author, `☂️ SFX Disini ☂️`, listSections, m)
} catch (e) {
