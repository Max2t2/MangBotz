...
switch (template) {
	...
	return await conn.sendListM(m.chat, bf, be, m)
        ...
        conn.send2ButtonVid(m.chat, cc.video, cc.title, cc.author, `Wm`, `.get ${cc.videoWM}`, `Audio`, `.get ${cc.audio}`, m)
            break
            case 'tiktok2':
        let cb2 = await fetch(`https://malesin.xyz/tiktok2?url=${one}`)
        let cc2 = await cb2.json()
        conn.send2ButtonVid(m.chat, giflogo, wm, author, `Ori`, `.get ${cc2.audio_original}`, `Audio`, `.get ${cc2.audio}`, m)
            break
           case 'pindl':
        let cb3 = await fetch(`https://malesin.xyz/pindl?url=${one}`)
        let cc3 = await cb3.json()
        conn.sendButtonImg(m.chat, cc3.result, wm, 'Nih.jpg', 'To Sticker', '.s', m)
            break
            case 'xnxxdl':
        let cb4 = await fetch(`https://malesin.xyz/xnxxdl?url=${one}`)
        let cc4 = await cb4.json()
        conn.send2ButtonVid(m.chat, cc4.result.files.high, cc4.result.title, cc4.result.duration + ' s', `Low`, `.get ${cc4.result.files.low}`, `HLS`, `.get ${cc4.result.files.HLS}`, m)
            break
            case 'pinterest':
